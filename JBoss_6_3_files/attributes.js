...
function (
	...
	function AnalyticsAttribute(args) {
		...
		/**
		 * Is this attribute overwritable once it's been set?
		 */
		this.overwritable = args.overwritable;
	...
	/**
	* Create a "master" attribute by passing in an array of Attribute names.
	* The values of the attributes so named will be used as the value of the
	* master attribute.
	*
	 * @memberof module:analytics/attributes
	 * @param {string[]} names The names of the attributes which comprise this
	 * master attribute
	 */
	function master(names) {
		...
		// If all values are undefined, return undefined, else return the
		// attributes' values
	...
	attribute_definitions = [
		 ...
		 *********************************************************************/
		{
			name : "ResourceID",
			getter : function () {
				return url.parse(location, url_patterns).ResourceID;
			},
			overwritable: true
		},
		{
			...
			getter : function () {
				var retval;
				if (attributes.ErrorPageName.value) {
					retval = attributes.ErrorPageName.value;
				} else {
					retval = url.parse(location, url_patterns).pageName;
				}
				return retval;
			},
			overwritable: true
		...
		},
		{
			name : "ResourceType",
			getter : function () {
				return url.parse(location, url_patterns).ResourceType;
			}
		},
		{
			name : "Platform",
			getter : function () {
				return url.parse(location, url_patterns).Platform;
			}
		},
		{
			name : "UseType",
			getter : function () {
				return url.parse(location, url_patterns).UseType;
			}
		},
		{
			name : "Collection",
			getter : function () {
				return url.parse(location, url_patterns).Collection;
			}
		},
		{
			name   : "URL",
			...
			overwritable: true // this attribute can be changed after being set initially
		},
		{
			name   : "TrueURL",
			getter : function () {
		...
		},
		{
			name : "Products",
			getter : function () {
				return url.parse(location, url_patterns).Product;
			},
			setter : function (v) {
			...
			lib.whenUserStatusReady(function () {
				...
				for (attr_key in attributes) {
					if (attributes.hasOwnProperty(attr_key)) {
						...
						// set value if an integrated app hasn't set it
						// already, OR if it's overwritable, it can be set
						// again
						if (typeof attr.value === "undefined" || attr.overwritable) {
							attr.set(attr.get());
						}
					}
				}
