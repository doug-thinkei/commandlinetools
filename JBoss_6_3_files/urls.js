 ...
 */
define(['analytics/separators'], function (separators, undefined) {
	"use strict";
	...
	/**
	 * Get the current document title, lowercase.
	 */
	function doc_title() {
		return require('analytics/attributes').get('ResourceTitle').toLowerCase();
	}
	...
	url_definitions = [
...

		{
			...
			get_page_name   : function () {
				...
				if (['documentation', 'knowledgebase', 'resources'].indexOf(subproduct) >= 0) {
					require('analytics/attributes').set('ToolName', 'Product Search'); // ATTREF
				}
...

		{
			...
			get_page_name   : function () {
				var group_id = this.params[2];
				require('analytics/attributes').set('GroupID', group_id); // ATTREF
		...
		},
		{
			...
			get_page_name   : function () {
				...
				require('analytics/attributes').set("Language", locale); // ATTREF
				return [this.Platform, this.ResourceType];
			}
		},
		{
			...
			get_page_name   : function () {
				...
				require('analytics/attributes').set("Language", locale); // ATTREF
				require('analytics/attributes').set("Products", product); // ATTREF
		...
		},
		{
			...
			get_page_name   : function () {
				...
				require('analytics/attributes').set('Products', product + ' ' + product_version); // ATTREF
				require('analytics/attributes').set('Language', locale); // ATTREF
		...
		},
		{
			...
			get_page_name   : function () {
				...
				require('analytics/attributes').set('Products', product + ' ' + product_version); // ATTREF
				require('analytics/attributes').set('ResourceTitle', documentation_title); // ATTREF
				require('analytics/attributes').set('ResourceType', 'docs-view'); // ATTREF
				require('analytics/attributes').set('ContentSubType', content_subtype); // ATTREF
				require('analytics/attributes').set('Language', locale); // ATTREF
				require('analytics/attributes').set('PublicationState', ['active', 'published']); // ATTREF
				//require('analytics/attributes').set('PublicationStatus', 'published');
				//require('analytics/attributes').set('PublicationState', 'active');
...

		{
			...
			get_resource_id : undefined,
			get_page_name   : function () {
				return [this.Platform, this.ResourceType, "new"];
			}
		},
		...
		********************/
		...
		{
			regex           : /^\/+support\/+policy\/+updates\/+([^\/+]+)\/*([?#].*)?$/,
			platform        : PLATFORMS.KPLATFORM,
			collection      : undefined,
			persona         : PERSONAS.CONSUMER,
			resource_type   : "support-policy",
			get_resource_id : undefined,
			get_page_name   : function () {
				return [this.Platform, this.ResourceType, doc_title()];
			}
